...
cvpr_tum::kfusionCPU::kfusionCPU(int rows, int cols, int encoding)
    : rows_(rows), cols_(cols), raw_depth_map_encoding_(encoding),
      ...
      robust_statistic_coefficient_(0.02f), regularization_(0.01f), angular_change_max_(0.05f), mov_change_max_(0.3f) {

    std::cout << "KFUSION CONSTRUCTOR WAS CALLED" << std::endl;

    const Vector3f volume_size = Vector3f::Constant(VOLUME_SIZE);
    const TsdfVolume::Vector3ui volume_resolution(TsdfVolume::VOLUME_X, TsdfVolume::VOLUME_Y, TsdfVolume::VOLUME_Z);
    ...
    reset();
    std::cout << "KFUSION CONSTRUCTOR WAS DONE" << std::endl;
...
//        cv::bilateralFilter(raw_depth_map_, depth_pyramid_[0], D, sigma_colour, sigma_space);
    ...
    //float rnorm = rodrigues2(Rcam).norm();
    //float tnorm = (tcam).norm();

    //if (rnorm > angular_change_max_) std::cout << "INTERPOLATION NEEDED!" << std::endl;
    //else if (tnorm > mov_change_max_) std::cout << "SHIFT NEEDED!" << std::endl;
