...
void SceneCloudView::updateCellsCloud(const cvpr_tum::TsdfVolume& volume)
{
    current_cloud_ptr_->points.clear();
...
void SceneCloudView::updateMesh(const cvpr_tum::TsdfVolume& volume)
{
    if (!marching_cubes_)
...
void SceneCloudView::renderScene(const cvpr_tum::TsdfVolume& volume, bool has_gpu)
{
    cloud_viewer_->removeAllPointClouds();
