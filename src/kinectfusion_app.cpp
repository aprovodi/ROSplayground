...
struct kfusionCpuApp {
        ...
        nh_private_(nh_private)
    {
        ...
        // init Kfusion
        nh_.param("world_size", world_size_, 5.0);
    ...
    void process_image_callback(const sensor_msgs::ImageConstPtr& msg) {
...

        {
            ...
            publishCameraTransformation((Eigen::Affine3d)kfusionCPU_.getCameraPose(), msg->header);
        }
    }
    ...
    void publishCameraTransformation(const Eigen::Affine3d& transform, const std_msgs::Header& h) {
        ...
        Eigen::Affine3d from_baselink_to_asus =
          //Eigen::AngleAxis<double>(-M_PI/4.0, Eigen::Vector3d(0, 0, 1)) *
          //Eigen::AngleAxis<double>(M_PI/2.0, Eigen::Vector3d(0, 1, 0)) *
          Eigen::AngleAxis<double>(M_PI/2.0, Eigen::Vector3d(0, 1, 0)) * Eigen::AngleAxis<double>(-M_PI/2.0, Eigen::Vector3d(0, 0, 1)) * Eigen::AngleAxis<double>(M_PI/4.0, Eigen::Vector3d(0, 1, 0)) * Eigen::Affine3d::Identity();


        // z points forward, xy are in image plane
	    Eigen::Affine3d transform_baselink = 
// Eigen::AngleAxis<double>(M_PI/4.0, Eigen::Vector3d(0, 0, 1)) *
  from_baselink_to_asus * transform * from_baselink_to_asus.inverse()// *
  //Eigen::AngleAxis<double>(M_PI/2.0, Eigen::Vector3d(1, 0, 0)) *
  //Eigen::AngleAxis<double>(M_PI/2.0, Eigen::Vector3d(0, 0, 1)) *
  //Eigen::AngleAxis<double>(-M_PI/4.0, Eigen::Vector3d(0, 0, 1))
          ...
          //Eigen::AngleAxis<double>(-M_PI/2.0, Eigen::Vector3d(0, 0, 1))
        ;
//transform_baselink.setIdentity();
        tf::StampedTransform tmp;
        tmp.frame_id_ = "world";
        tmp.child_frame_id_ = "camera_link";
        tmp.stamp_ = h.stamp;

        tf::TransformEigenToTF(transform_baselink, tmp);
        tf::poseTFToMsg(tmp, msg->pose.pose);

        static tf::TransformBroadcaster tb;
        tb.sendTransform(tmp);
