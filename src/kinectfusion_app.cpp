...
struct SceneCloudView
{
        ...
        cloud_viewer_("Scene Cloud Viewer")
    {
        ...
        cloud_viewer_.addCoordinateSystem(0.5, 1);
    ...
    void show(kfusionCPU& kfusion)
    {
        ...
        for(auto i = vmap.origin();
                i < (vmap.origin() + vmap.num_elements());
                ++i)
                cloud_ptr_->points.push_back(pcl::PointXYZ(i->x, i->y, i->z));
        ...
        public:
            ...
            void process_image_callback(const sensor_msgs::ImageConstPtr& msg)
            {
                ...
                sensor_msgs::ImagePtr raw_image =
                        map_to_image(kfusionCPU_.get_raw_depth_map(), raw_depth_map->encoding);
                raw_depth_publisher_.publish(raw_image);
                ROS_DEBUG("Done!");

                ROS_DEBUG("Publishing filtered depth map as image...");
                sensor_msgs::ImagePtr filtered_image = map_to_image(kfusionCPU_.filtered_depth_map(),
                                                                    raw_image->encoding);
